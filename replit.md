...
- **AI Fraud Detection (T013)**: Each submitted response receives a `fraud_score` (0–100) computed at submission time based on GPS accuracy, interview duration, time-of-day, GPS coordinate clustering, audio duration, and pre-existing suspicious status. Scores are displayed as colored "IA X" badges in the audit list and a detailed risk panel in the audit detail dialog.
- **WhatsApp Supervisor Alerts (T010)**: Uses Twilio to send WhatsApp messages to a configured supervisor number. Three triggers: (1) geofence violation, (2) survey quota 100% reached, (3) idle interviewer detected (no response for 30+ minutes, checked every 5 minutes by a background job in `server/index.ts`). Phone number configured per-org in Settings → Alertas tab (`whatsapp_phone` column on organizations table, E.164 format).
- **Wave Comparison (T007)**: "Comparar Ondas" tab in survey results lets users select another survey from the same org and compare vote intention results side-by-side with a grouped bar chart and a Δ (delta) table showing point-percentage differences. Surveys can have a `wave_label` text column to identify survey rounds.
- **ES State Municipality Map (T012)**: Page `/org/:orgId/state-map` shows a choropleth map of all Espírito Santo municipalities using IBGE's public GeoJSON API. Municipalities with survey data are colored by the leading candidate's percentage. Click a municipality to see survey details in a side panel. Backend endpoint `/api/organizations/:orgId/state-map-data` aggregates vote intention data per `geofenceCity`.
